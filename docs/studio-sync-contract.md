...
| `asset_folder_id` `asset_folder_url` | Optional safe fallback for an older job whose own folder link is empty: Soleia supplies the one folder shared by all attached packets/proposals. If attached records disagree about the folder, both fields are `null`; consumers must not choose one by name or date. |
