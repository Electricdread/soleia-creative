...
-- Update cached_clips policies - drop permissive ones
...
-- Create admin-only policies for cached_clips
CREATE POLICY "Admins can insert clips"
ON public.cached_clips FOR INSERT
WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can update clips"
ON public.cached_clips FOR UPDATE
USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can delete clips"
ON public.cached_clips FOR DELETE
USING (public.has_role(auth.uid(), 'admin'));
