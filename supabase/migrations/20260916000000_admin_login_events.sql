-- Records each real admin sign-in (email, IP, user agent) so the site owner
-- can see who is signing into /admin and from where. Rows are written only
-- by the log-admin-login edge function using the service role, which
-- bypasses RLS — there is no insert policy for anon/authenticated.
