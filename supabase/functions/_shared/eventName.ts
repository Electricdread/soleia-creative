/**
 * Deno copy of `src/lib/eventName.ts`.
 *
 * Edge functions cannot import from `src/`, so the booking-name rule lives twice.
 * `src/lib/eventName.deno.test.ts` reads this file and fails unless everything
 * from the first `export` is byte-identical to the browser copy.
 *
 * If these drift, a booking reads one name on Soleia's calendar and another in
 * Google Calendar, DreamlinkX OS and the production folder.
 *
 * Change one, change both.
 */
import { format, isValid, parseISO } from 'npm:date-fns@3.6.0';
