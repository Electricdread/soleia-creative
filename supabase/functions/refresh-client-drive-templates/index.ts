...
async function findChildFolder(parentId: string, name: string) {
  ...
  const list = await gw(`/drive/v3/files?q=${q}&fields=files(id,name)&pageSize=1`, { method: 'GET' });
  return list?.files?.[0]?.id ?? null;
}

async function findFiles(parentId: string, name: string) {
  const q = encodeURIComponent(
    `name='${name.replace(/'/g, "\\'")}' and '${parentId}' in parents and trashed=false`,
  );
  const list = await gw(`/drive/v3/files?q=${q}&fields=files(id,name)&pageSize=10`, { method: 'GET' });
...
async function uploadZip(parentId: string, zipBytes: Uint8Array) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const p of proposals ?? []) {
      ...
      try {
        const subfolderId = await findChildFolder(p.drive_folder_id as string, SUBFOLDER_NAME);
        if (!subfolderId) {
          summary.skipped_no_subfolder += 1;
          continue;
        }

        const existing = await findFiles(subfolderId, ZIP_NAME);
        for (const f of existing) {
          await deleteFile(f.id);
        }

        await uploadZip(subfolderId, zipBytes);
        summary.updated += 1;
      } catch (e) {
        summary.errors.push({
