...
export async function generateSelectionsPdf(
  ...
  // Soleia branding - centered
  const poweredByText = 'Soleia Creative Team';
  pdf.setFontSize(7);
  pdf.setFont(FONTS.body.family, FONTS.body.style);
  ...
  if (soleiaIconBase64) {
    try {
      pdf.addImage(soleiaIconBase64, 'PNG', brandX + poweredByWidth + 2, footerY - 4, iconWidth, iconWidth);
    } catch (e) {
      console.error('Failed to add Soleia icon:', e);
    }
  }
...
export async function generateSelectionsPdfDataUri(
  ...
  // ========== FOOTER WITH SHOWBLOX BRANDING ==========
  const footerY = pageHeight - 12;
  
  // Load ShowBlox icon for footer
  ...
  if (showbloxBase64) {
    try {
      pdf.addImage(showbloxBase64, 'PNG', brandX + poweredByWidth + 2, footerY - 4, iconWidth, iconWidth);
    } catch (e) {
