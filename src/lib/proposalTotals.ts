 ...
 *   - Discount: a proposal may carry one, as a percentage or a flat amount off.
 *     `calcProposalTotal` returns what the client actually owes, discount
 *     already taken off — so a caller that forgets the discount cannot quote a
 *     figure above the agreed one. Use `calcSubtotal` for the pre-discount
 *     figure a breakdown needs.
 */
