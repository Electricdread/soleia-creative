...
describe('unsigned',()=>{ it('one page',async()=>{ const r:any = await generateProposalPdf({event_name:'MOC&CO x ZAXBYS',client_name:'Zaxbys',status:'sent'} as any, ITEMS as any, [{phase:'a',description:'b',duration:'1w'},{phase:'c',description:'d',duration:'2w'},{phase:'e',description:'f',duration:'3w'}] as any, null, [], {returnBase64:true}); const p=count(r.base64); console.log('PAGES',p); expect(p).toBe(1); }); });
