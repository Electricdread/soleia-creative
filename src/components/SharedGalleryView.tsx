...
import { Search, Sparkles, Calendar, Users, Play, X, MessageSquare, MapPin, ChevronDown, ChevronUp, Check, Star, Loader2 } from 'lucide-react';
...
const SharedGalleryView: React.FC<SharedGalleryViewProps> = ({
  ...
  // Convert sessionClips to ArtlistClip format for display
  const clips: ArtlistClip[] = sessionClips.map(sc => ({
    id: sc.id,
    title: sc.title,
    thumbnail: sc.thumbnail || '',
    previewUrl: sc.preview_url || sc.video_url || '',
    videoUrl: sc.video_url || '',
    duration: sc.duration || '',
    resolution: sc.resolution || '4K',
    category: sc.category,
    sourceUrl: sc.video_url || '',
  }));
        ...
        <div className="mb-6 md:mb-8 flex flex-wrap items-center gap-3 md:gap-4">
          {filteredClips.length > 0 && (
            <span className="text-muted-foreground text-sm tracking-wide">
              {filteredClips.length} clips curated for your event
            </span>
          )}
        </div>
          ...
          {filteredClips.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <h3 className="text-2xl font-bold text-foreground mb-3 tracking-tight">
                {searchQuery ? 'No matching clips' : 'No clips available'}
              </h3>
              <p className="text-muted-foreground mb-6">
                {searchQuery ? 'Try a different search term' : 'Your collection is being prepared'}
              </p>
            </div>
          ) : (
            <motion.div 
                ...
                return (
                  ...
                  >
                    <ClipThumbnail
                      ...
                      categoryGradient={categoryGradients[clip.category] || categoryGradients['abstract']}
                    />
