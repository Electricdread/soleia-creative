...
import { Check, Monitor, Sun, Layers } from 'lucide-react';
...
export function LEDZoneCard({ zone, isSelected, onToggle }: LEDZoneCardProps) {
  return (
    ...
    >
      <Card
        className={`relative cursor-pointer transition-all duration-300 touch-manipulation overflow-hidden ${
          isSelected
        ...
        <CardContent className="p-4 sm:p-5">
          ...
          <div className="flex items-start gap-3 mb-3">
            <div className={`p-2 rounded-lg ${isSelected ? 'bg-primary/20' : 'bg-muted/50'}`}>
              {subcategoryIcons[zone.subcategory]}
            </div>
          ...
          <p className="text-xs text-muted-foreground font-medium mb-2">
            {ZONE_SUBCATEGORY_LABELS[zone.subcategory]}
          </p>
