...
export function EventAttachments({ eventUid }: { eventUid: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <Button size="sm" onClick={() => fileRef.current?.click()} disabled={uploading} className="bg-[#c49a3c] hover:bg-[#b08a30] text-white text-xs gap-1.5">
        {uploading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Upload className="w-3 h-3" />} Upload Document
      </Button>

      {attachments.length === 0 && <p className="text-xs text-[#b5ab9a] italic">No documents attached</p>}

      <div className="space-y-2">
        {attachments.map((att) => (
          <div key={att.id} className="flex items-center gap-3 bg-[#faf8f5] border border-[#e8e2d8] rounded-lg p-3 group">
            <FileText className="w-5 h-5 text-[#c49a3c] shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-[#3d3629] truncate">{att.file_name}</p>
              <p className="text-[10px] text-[#b5ab9a]">
                {formatSize(att.file_size)} · {format(new Date(att.created_at), 'MMM d, yyyy')}
              </p>
            </div>
            <a href={att.file_url} target="_blank" rel="noopener noreferrer" className="text-[#c49a3c] hover:text-[#b08a30]">
              <ExternalLink className="w-4 h-4" />
            </a>
            <button onClick={() => deleteAttachment(att)} className="opacity-0 group-hover:opacity-100 text-[#b5ab9a] hover:text-[#b05a5a] transition-opacity">
